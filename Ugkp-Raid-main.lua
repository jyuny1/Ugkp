...
function Ugkp_RaidMain_OnEvent(event)
	...
	elseif ( event == "CHAT_MSG_LOOT" and Gkp_CurrentRaidId~=0) then
		if(UGKP_PARTY_STUFF[UgkpRaid_GetCurrentRaid]["Status"]==1) then
			local sPlayer, sLink;
			local iStart, iEnd, sPlayerName, sItem = string.find(arg1, UgkpRaid_lang_ReceivesLoot1);
			
			local s, e, tesItem, itemNum = string.find(arg1, UgkpRaid_lang_ReceivesLoot3);
			
			local itemcount=1;
			if(itemNum) then
				iStart, iEnd, sPlayerName, sItem = string.find(arg1, UgkpRaid_lang_ReceivesLoot4);
				itemcount=itemNum;
			end
			...
			if ( sPlayerName ) then
				...
				local iStart, iEnd, sItem = string.find(arg1, UgkpRaid_lang_ReceivesLoot2);
